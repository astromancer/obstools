...
class FrameProcessorGUI(VideoDisplay, LoggingMixin):
    ...
    # TODO: should inherit from BLittingHelper class
    ...
    def update(self, i, draw=True):
        ...
        # get image
        draw_list = VideoDisplay.update(self, i, draw)
        i = self.frame
    ...
    def add_legend2(self):
        ...
        # Apertures from model fits
        apStar = Ellipse((0, 0), 1, 1, **self.apStarProp)  # aperture
        apSkyin = Ellipse((0, 0), 1, 1, **self.apSkyProp)
        apSkyout = Ellipse((0, 0), 1, 1, **self.apSkyProp)
        rect = Rectangle((0, 0), 1.4, 1.4, **self.winProp)  # windows
        xfit = Line2D([0], [0], **self.xfitProp)  # fit position

        # Circular aps
        xcom = Line2D([0], [0], **self.xcomProp)  # CoM markers
