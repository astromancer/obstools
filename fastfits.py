...
#from pyfits.base import _BaseHDU

#import pylab as plt
#from matplotlib.transforms import  blended_transform_factory as btf

from recipes.iter import first_true_idx
from recipes.dict import DefaultOrderedDict #lmap,
from myio import parsetolist
...
#from superplot import hist, lcplot
...
class FITSFrame():
    ...
    def __init__(self, filename):
        ...
        self.master_header = header = pyfits.Header.fromstring( header_str )
        
        #check if data is 3D
        nax = header['naxis']
        if nax != 3:
            raise TypeError('{} only works with 3D data!'.format(self.__class__.__name__))
        
        #figure out the size of a data block
        #self.shape = (header['naxis%i'%i] for i in range(1,nax+1))
        self.shape =  header['naxis1'], header['naxis2'], header['naxis3']
        self.ishape = nax1, nax2 = self.shape[:-1]
...
def fetchframe(filename, frame):
    return FITSFrame(filename)[frame]
...
class FastExtractor(object):
    ...
    def setup(self, filename):
        ...
        bitpix = header['bitpix']
        self.data_size = dsize = abs(bitpix) * self.nax1 * self.nax2 // 8

        #now assume fits complience -- i.e. file is divided in chuncks of 2880 bytes
        BLOCK = self.BLOCK
        self.data_step = int( np.ceil((dsize+hsize)/BLOCK)*BLOCK )
        self.nextend = Next = mheader['nextend']
        
        #expected number of frames to extract (assuming all have identical structure!)
        self.Ntot = (self.Nfiles*Next - self.start) // self.step
    ...
    def loop(self):
        ...
        start, stop, step = self.start, self.stop, self.step
        dstart, dstep, dsize = self.data_start, self.data_step, self.data_size
