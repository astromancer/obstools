...
#from SHOC_user_input import InputCallbackLoop
...
class ApertureInteraction( AccessRadii ):
    ...
    def gen_phot_aps(self):
        ...
        coords = np.array( list(zip(*itertools.repeat(self.stars.coords, Naps))) ).reshape(-1,2)
...
class StarSelector(MplMultiTab):
    ...
    def __init__(self, im_fns, coord_fns=None, offsets=None, show_blend=1):
        ...
        self.has_phot, _ = self.filenames.look_for('mag')
        ...
        pool = mpc.Pool()
        self.figures = pool.map( setup_figure_geometry, range(len(im_fns)+show_blend) )

        for i, fn in enumerate(im_fns):

            api = ApertureInteraction( i, self.figures[i] )
            api.load_image( fn, WCS=self.WCS )
    ...
    def propagate(self, TF=None, whence='current', from_coo=None, narcisistic=False, _pr=1, **snapkw): #NEEDS api index ---> prop from this one
        ...
        def warn_for_unconvergent(cluster, nidx, nargs):
            ...
            unconvergent, ok = partitionmore(lambda i : i is None, cluster, nidx, nargs)
    ...
    def write_coo( self, TF=0 ):
        ...
        if not yn:
            ...
            #header, data = partition( lambda s: s.startswith('#'), open(fn,'r') )
...
if __name__ == '__main__':
    ...
    reduction_path = path# os.path.join( path, 'ReducedData' )
