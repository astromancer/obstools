...
def opt_factory(p):
    # initialize
    return [EllipseOptimizer, CircleOptimizer][len(p) == 1]()
    ...
    def optimal_aperture_photometry(self, i, data, residu, coords, tracker,
        ...
        for g, (name, labels) in enumerate(tracker.groups.items()):
            ...
            if flag != 1:  # there was an error or no convergence
                ...
                # optimization failed for this group. if no convergence for this
                # frame or previous, fall back to p0
            ...
            # ie. optimization was at least attempted
            elif len(p) == 1:  # circle
                a, = b, = p
                theta = 0
